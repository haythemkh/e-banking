...
namespace agence3
{
    class MesMethodes : MarshalByRefObject, iagence.IAgence3
    {
        SqlConnection sc = new SqlConnection("Data Source=Xar;Initial Catalog=agence3;Integrated Security=True");
        SqlCommand cmd;
        SqlDataReader rdr;

        string IAgence3.getSolde(string numCompte)
        {
        ...
        string IAgence3.getHistoriqueSoldes(string numCompte)
        {
            if (sc != null && sc.State == ConnectionState.Closed)
                sc.Open();
        ...
        string IAgence3.getHistoriqueMouvements(string numCompte)
        {
            if (sc != null && sc.State == ConnectionState.Closed)
                sc.Open();
        ...
        string IAgence3.effectuerVirement(string numCompte, string beneficiaire, string mnt, string statut)
        {
        ...
        string IAgence3.retrait(string numCompte, string mnt)
        {
            decimal montant = decimal.Parse(mnt, CultureInfo.InvariantCulture);
            String strMontant = montant.ToString();
            string dateNow = DateTime.Now.ToString("yyyy-MM-dd");
        ...
        string IAgence3.verser(string numCompte, string mnt)
        {
            decimal montant = decimal.Parse(mnt, CultureInfo.InvariantCulture);
            String strMontant = montant.ToString();
            string dateNow = DateTime.Now.ToString("yyyy-MM-dd");
        ...
        string IAgence3.connecter(string numCompte, string motDePasse)
        {
            if (sc != null && sc.State == ConnectionState.Closed)
                sc.Open();
            try
        ...
        string IAgence3.getInfos(string numCompte)
        {
            if (sc != null && sc.State == ConnectionState.Closed)
                sc.Open();
            string ret = "";
        ...
        string IAgence3.verifierCompteBeneficiaire(string beneficiaire)
        {
            if (sc != null && sc.State == ConnectionState.Closed)
                sc.Open();
            try
            {
                ...
                rdr = cmd.ExecuteReader();
                if (!beneficiaire.Trim().Equals("1032345678"))
                {
                    rdr.Close();
