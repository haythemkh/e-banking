...
namespace agence2
{
    class MesMethodes : MarshalByRefObject, iagence.IAgence2
    {
        SqlConnection sc = new SqlConnection("Data Source=Xar;Initial Catalog=agence2;Integrated Security=True");
        SqlCommand cmd;
        SqlDataReader rdr;

        string IAgence2.getSolde(string numCompte)
        {
        ...
        string IAgence2.getHistoriqueSoldes(string numCompte)
        {
            if (sc != null && sc.State == ConnectionState.Closed)
                sc.Open();
        ...
        string IAgence2.getHistoriqueMouvements(string numCompte)
        {
            if (sc != null && sc.State == ConnectionState.Closed)
                sc.Open();
        ...
        string IAgence2.effectuerVirement(string numCompte, string beneficiaire, string mnt, string statut)
        {
        ...
        string IAgence2.retrait(string numCompte, string mnt)
        {
            decimal montant = decimal.Parse(mnt, CultureInfo.InvariantCulture);
            String strMontant = montant.ToString();
            string dateNow = DateTime.Now.ToString("yyyy-MM-dd");
        ...
        string IAgence2.verser(string numCompte, string mnt)
        {
            decimal montant = decimal.Parse(mnt, CultureInfo.InvariantCulture);
            String strMontant = montant.ToString();
            string dateNow = DateTime.Now.ToString("yyyy-MM-dd");
        ...
        string IAgence2.connecter(string numCompte, string motDePasse)
        {
            if (sc != null && sc.State == ConnectionState.Closed)
                sc.Open();
            try
        ...
        string IAgence2.getInfos(string numCompte)
        {
            if (sc != null && sc.State == ConnectionState.Closed)
                sc.Open();
            string ret = "";
        ...
        string IAgence2.verifierCompteBeneficiaire(string beneficiaire)
        {
            if (sc != null && sc.State == ConnectionState.Closed)
                sc.Open();
            try
            {
                ...
                rdr = cmd.ExecuteReader();
                if (!beneficiaire.Trim().Equals("1022345678"))
                {
                    rdr.Close();
